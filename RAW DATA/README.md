...
- `alcohol_freq` is the **only** column with missing values (30,083 rows, ≈30.1%).
- `age` includes 0, so some rows are dependents/minors, not just adults.
- Cost/claims fields (`annual_medical_cost`, `total_claims_paid`, `avg_claim_amount`) are heavily right-skewed — consider log transform or outlier capping for modeling.
- `claims_count = 0` (and related zeros) are structural — no claims filed — not missing data.
- `chronic_count` is derived (sum of the 10 binary flags); useful as a quick severity score.

## Suggested Star-Schema Split

For a relational model / BI dashboard, join on `person_id`:

- **Fact table:** `risk_score`, `annual_medical_cost`, `annual_premium`, `monthly_premium`, `claims_count`, `avg_claim_amount`, `total_claims_paid`, `is_high_risk`
- **Dim: Demographics** — age, sex, region, urban_rural, income, education, marital_status, employment_status, household_size, dependents
- **Dim: Health Profile** — bmi, smoker, alcohol_freq, vitals, chronic_count + 10 condition flags
- **Dim: Policy** — plan_type, network_tier, deductible, copay, policy_term_years, policy_changes_last_2yrs, provider_quality
- **Dim: Utilization** — visits_last_year, hospitalizations_last_3yrs, days_hospitalized_last_3yrs, medication_count, the 5 `proc_*` counts, had_major_procedure

This maps cleanly to a Demographics / Risk Analysis / Customers / Policy & Ops dashboard structure.
